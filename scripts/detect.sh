...
if [ -z "$1" ]; then
    echo "Usage: ./detect.sh <video_path> [--checkpoint <path>] [--threshold <float>]"
    exit 1
fi

python main.py detect --config configs/default.yaml --checkpoint checkpoints/best_model.pth --video "$1" "${@:2}"
